...
class WomenStartup extends CI_Controller {


    public function index() {
        $config = array();
        $where = array('action'=>'accept','delstatus'=>1,'is_women'=>1,'is_screened'=>1,'is_graduate'=>0);
        $config["base_url"] = base_url() . "WomenStartup";
        ...
        $this->load->view('pages/women-startup', $data);
    }
    ...
    public function updateStartup()
    {
        extract($_POST);
            ...
            if($rs)
            {
              ...
              redirect(base_url().'WomenStartup/EditStartup?id='.$updateid);
            }
            else 
            {
