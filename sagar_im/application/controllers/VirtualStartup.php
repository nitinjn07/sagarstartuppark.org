...
class VirtualStartup extends CI_Controller {


    public function index() {
        $config = array();
        $where = array('action'=>'accept','delstatus'=>1,'startup_type'=>'Virtual','is_screened'=>1,'is_graduate'=>0);
        $config["base_url"] = base_url() . "VirtualStartup";
        ...
        $this->load->view('pages/virtual-startup', $data);
    }
    ...
    public function updateStartup()
    {
        extract($_POST);
            ...
            if($rs)
            {
              ...
              redirect(base_url().'VirtualStartup/EditStartup?id='.$updateid);
            }
            else 
            {
